...
std::string execute_cgi(const std::string& script_path, const std::string& method, const std::string& query_string, const std::string& post_data) {
    int pipefd[2];
    ...
    if (pid == 0) { // Child process
        ...
        // Execute the CGI script using the PHP interpreter
        if (execl("/usr/bin/php", "/usr/bin/php", script_path.c_str(), NULL) == -1) {
            perror("execl failed");
            exit(1);
        }
...
int main() {
    ...
    while (true) {
        ...
        if (bytes_read > 0) {
            ...
            // Check if the file is a CGI script (e.g., .php)
            if (path.find(".php") != std::string::npos) {
                std::string cgi_output = execute_cgi("." + path, method, query_string, post_data);
